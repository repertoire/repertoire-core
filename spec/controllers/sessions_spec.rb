...
describe "RepertoireCore::Users (controller)" do
  ...
  it "should have a named route :login" do
    @controller.url(:login).should == "/repertoire_core/login"
  end
  
  it "should have route to Sessions#destroy from '/logout' via delete" do
    request_to("/repertoire_core/logout", :delete) do |params|
      params[:controller].should == "Sessions"
  ...
  it "should route to Sessions#destroy from '/logout' via get" do
    request_to("/repertoire_core/logout") do |params|
      params[:controller].should == "Sessions" 
      params[:action].should     == "destroy"
    end
  end
  ...
  it 'logins and redirects' do
    controller = post "/repertoire_core/login", :email => 'bill@globe.com', :password => 'tiger'
  ...
  it 'fails login and does not redirect' do
    controller = post "/repertoire_core/login", :email => 'bill@globe.com', :password => 'bad password'
    controller.session[:user].should be_nil
    controller.should be_successful
  end

  it 'logs out' do
    controller = get("/repertoire_core/logout"){|controller| controller.stub!(:current_user).and_return(@quentin) }
    controller.session[:user].should be_nil
    controller.should redirect
  end

  it 'remembers me' do
    controller = post "/repertoire_core/login", :email => 'bill@globe.com', :password => 'tiger', :remember_me => "1"
    controller.cookies["auth_token"].should_not be_nil
  end
 
  it 'does not remember me' do
    controller = post "/repertoire_core/login", :email => 'bill@globe.com', :password => 'tiger', :remember_me => "0"
    controller.cookies["auth_token"].should be_nil
  end
  
  it 'deletes token on logout' do
    controller = get("/repertoire_core/logout") {|request| request.stub!(:current_user).and_return(@quentin) }
    controller.cookies["auth_token"].should == nil
  end
  
  it 'logs in with cookie' do
    @quentin.remember_me
    controller = get "/repertoire_core/login" do |c|
